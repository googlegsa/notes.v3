...
********************************************************************** -->
 ...
 Global Style (do not customize)
	...
	<xsl:template name="style">
		<style>
			<xsl:comment>
body,td,div,.p,a,.d,.s{font-family:
				<xsl:value-of select="$global_font"/>}
body,td,div,.p,a,.d{font-size: 
				<xsl:value-of select="$global_font_size"/>}
body,div,td,.p,.s{color:
				<xsl:value-of select="$global_text_color"/>}
body,.d,.p,.s{background-color:
				<xsl:value-of select="$global_bg_color"/>}
.s{font-size: 
				<xsl:value-of select="$res_snippet_size"/>}
.g{margin-top: 1em; margin-bottom: 1em}
.s td{width:34em}
.l{font-size: 
				<xsl:value-of select="$res_title_size"/>}
.l{color: 
				<xsl:value-of select="$res_title_color"/>}
a:link,.w,.w a:link{color:
				<xsl:value-of select="$global_link_color"/>}
.f,.f:link,.f a:link{color:
				<xsl:value-of select="$faint_color"/>}
a:visited,.f a:visited{color:
				<xsl:value-of select="$global_vlink_color"/>}
a:active,.f a:active{color:
				<xsl:value-of select="$global_alink_color"/>}
.t{color:
				<xsl:value-of select="$sep_bar_text_color"/>}
.t{background-color:
				<xsl:value-of select="$sep_bar_bg_color"/>}
.z{display:none}
.i,.i:link{color:#a90a08}
.a,.a:link{color:
				<xsl:value-of select="$res_url_color"/>}
div.n {margin-top: 1ex}
.n a{font-size: 10pt; color:
				<xsl:value-of select="$global_text_color"/>}
...
div.oneboxResults {max-height:150px;overflow:hidden;}
 ...
 URL variables (do not customize)
  ...
  select="/GSP/PARAM[@name='testSearch']/@value"/>
	...
	<xsl:variable name="home_url">search?<xsl:value-of select="$base_url"/>&amp;proxycustom=&lt;HOME/&gt;
	</xsl:variable>
  ...
  select="/GSP/PARAM[(@name != 'q') and
	...
	<xsl:variable name="filter_url">search?<xsl:for-each select="/GSP/PARAM[(@name != 'filter') and
                     (@name != 'epoch' or $is_test_search != '') and
                     not(starts-with(@name, 'metabased_'))]">
			<xsl:value-of select="@name"/>
			<xsl:text>=</xsl:text>
	...
	<xsl:variable name="adv_search_url">search?<xsl:value-of select="$search_url"/>&amp;proxycustom=&lt;ADVANCED/&gt;</xsl:variable>
	...
	<xsl:variable name="swr_search_url">search?<xsl:value-of select="$search_url"/>&amp;swrnum=
...
// </xsl:comment>
			...
			<body onload="setFocus()" dir="ltr">
				...
				<form method="get" action="search" name="f">
					...
					<!--====Customisation to disable search results filtering-->
      ...
      //</xsl:comment>
        ...
        function expand (thistag, tag) {
    		...
    		if (styleObj.display=='none'){
				styleObj.display='';
				tag.innerHTML = "[Hide links]";
			}
			else {
				styleObj.display='none';
				tag.innerHTML = "[Show links]";
			}
		}
      //</xsl:comment>
			</script>
 ...
 Output all results
	...
	<xsl:template name="results">
        ...
        In order to show you the most relevant results,    we have omitted some entries very similar to the 
						<xsl:value-of select="RES/@EN"/> already    displayed.
						<br/>If you like, you can 
						<a href="{$filter_url}0">    repeat the search with the omitted results included</a>.
					</i>
				</p>
			</xsl:if>
		</div>
 ...
 Truncation functions (do not customize)
  ...
  A single result (do not customize)
	...
	<xsl:template match="R">
		...
		<xsl:variable name="row" select="@N"/>
				...
				<font size="-2">
					<b>
						<xsl:choose>
							...
							<xsl:when test="MT[@N='Form']/@V='Document'">[NOTES]</xsl:when>
        ...
        [
					...
					<xsl:choose>
						<xsl:when test="T">
							<xsl:call-template name="reformat_keyword">
								<xsl:with-param name="orig_string" select="MT[@N='Topic']/@V"/>
							</xsl:call-template>
						</xsl:when>
						<xsl:otherwise>
			...
			<table cellpadding="0" cellspacing="0" border="0">
				<tr>
					<td class="s">
						<xsl:choose>
							<xsl:when test="MT[@N='Form']/@V='Document'">
								<xsl:call-template name="reformat_keyword">
									<xsl:with-param name="orig_string" select="MT[@N='Summary']/@V"/>
								</xsl:call-template>
							</xsl:when>
							<xsl:otherwise>
						...
						<xsl:if test="$show_meta_tags != '0'">
							<div id="{$row}" class="f" style="font-size:8pt" onclick="expand('{$row}-links', this);">
						[Show links]
							</div>
							<span id="{$row}-links" style="display:'none';">
								<xsl:choose>
									<xsl:when test="MT[@N='Form']/@V='Document'">
										<span class="f" style="font-size:8pt">
										Document:
										</span>
										<font color="{$res_url_color}" style="font-size:8pt">
											<xsl:text disable-output-escaping='yes'>&lt;a href="</xsl:text>
											<xsl:value-of select="MT[@N='WebLink']/@V"/>
											<xsl:text disable-output-escaping='yes'>"&gt;[Browser]&lt;/a&gt;  </xsl:text>
											<xsl:text disable-output-escaping='yes'>&lt;a href="</xsl:text>
											<xsl:value-of select="MT[@N='NotesLink']/@V"/>
											<xsl:text disable-output-escaping='yes'>"&gt;[Notes Client]&lt;/a&gt;</xsl:text>
										</font>
										<xsl:if test="MT[@N='Attachments']/@V !='None'">
											<br/>
											<span class="f" style="font-size:8pt">
											All attachments:
											</span>
											<font color="{$res_url_color}" style="font-size:8pt">
												<xsl:call-template name="tokenize">
													<xsl:with-param name="string" select="MT[@N='Attachments']/@V"/>
          ...
          [
            ...
            $search_url}">More results from 
								<xsl:value-of select="$crowded_display_url"/>
							</a>
          ]
	...
	<xsl:template name="analytics">
		<xsl:if test="string-length($analytics_account) != '0'">
			...
			<script type="text/javascript">
				<xsl:comment>
      _uacct = "
					<xsl:value-of select='$analytics_account'/>";
      urchinTracker();
      //
				</xsl:comment>
			</script>
	...
	<xsl:template name="_tokenize-delimiters">
		...
		<xsl:choose>
			...
			<xsl:when test="not($delimiter)">
				<token>
					<xsl:text disable-output-escaping='yes'>&lt;a href="</xsl:text>
					<xsl:value-of select="MT[@N='DocPath']/@V"/>
